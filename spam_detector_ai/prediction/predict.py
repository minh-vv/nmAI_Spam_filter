...
def get_model_path(model_type):
    # Xác định thư mục của file này
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Giả định rằng thư mục spam_detector_ai nằm phía trên thư mục hiện tại
    base_dir = os.path.dirname(current_dir)

    # Định nghĩa đường dẫn tương đối cho từng loại mô hình bằng cách sử dụng dictionary
    paths_map = {
    ...
    # Lấy đường dẫn tương đối của model và vectoriser từ dictionary
    relative_path_model, relative_path_vectoriser = paths_map.get(model_type)

    if relative_path_model and relative_path_vectoriser:
        # Xây dựng đường dẫn tuyệt đối bằng cách kết hợp thư mục cơ sở với đường dẫn tương đối
        ...
        # Báo lỗi nếu loại model không hợp lệ
        raise ValueError(f"Invalid model type: {model_type}")


class SpamDetector:
    """Lớp này được sử dụng để phát hiện xem một tin nhắn có phải là spam hay không."""

    def __init__(self, model_type=ClassifierType.NAIVE_BAYES):
        # Ánh xạ giữa loại phân loại và các đối tượng phân loại tương ứng
        classifier_map = {
